...
while True:
    if run:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # Delete an enemy row if it is empty
                [enemy_rows.pop(enemy_rows.index(enemies)) 
                 for enemies in enemy_rows if len(enemies) == 0]
                ...
                def change_enemies_level():
                    [enemy.change_level() for enemies in enemy_rows for enemy in enemies]

                # move all enemies in the same direction
                [enemy.change_position() for enemies in enemy_rows for enemy in enemies]
                    ...
                    for enemy in enemies:
                        if enemy.laser is not None: # Does Laser exist
                            if enemy.laser.position_y + enemy.laser.vel <= 600: # If the Laser is out of the Map
                                if enemy.laser.collision(player) == True: # Does the laser collide with player
                                    player.lives-=1
                                    enemy.laser = None
    ...
    def end_game(result):
        end_label = main_font.render(result, 1, (255, 255, 255))
        screen.blit(end_label, (230,260))
    ...
    [enemy.draw(screen) for enemies in enemy_rows for enemy in enemies]
    player.draw(screen)
    player.handle_keys()
